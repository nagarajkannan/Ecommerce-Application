...
const Collection = () => {
  ...
  return (
    <div className='min-h-screen rounded mb-10 px-4 sm:px-6 lg:px-20 py-10'>
      <div className='flex flex-col sm:flex-row gap-6 lg:gap-10'>

        {/* Sidebar Filters */}
        <div className='w-full sm:w-[280px] bg-gray-50 p-4 rounded-md shadow-md'>
          <div className='flex items-center justify-between sm:block'>
            <p className='text-2xl font-bold'>FILTERS</p>
            <img
              src={assets.dropdown_icon}
              className={`h-4 sm:hidden cursor-pointer transition-transform duration-200 ${showfilter ? 'rotate-90' : ''}`}
              onClick={() => setShowFilter(!showfilter)}
              alt="Toggle Filter"
            />
          </div>

          <div className={`mt-6 space-y-6 ${showfilter ? 'block' : 'hidden'} sm:block transition-all`}>
            {/* Category Filter */}
            <div className='bg-gray-100 pl-5 py-3 rounded-md'>
              <p className='mb-3 text-xl sm:text-2xl font-medium text-gray-800'>CATEGORIES</p>
            ...
            {/* Sub-category Filter */}
            <div className='bg-gray-100 pl-5 py-3 rounded-md'>
              <p className='mb-3 text-xl sm:text-2xl font-medium text-gray-800'>TYPE</p>
              <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
                {['Topwear', 'Bottomwear', 'Winterwear'].map(sub => (
                  <label key={sub} className='flex items-center gap-2 hover:bg-gray-200 p-1 rounded cursor-pointer'>
                    <input className="w-4 h-4" type='checkbox' value={sub} onChange={toggleSubCategory} />
                    {sub}
                  </label>
                ))}
              </div>
